...
<body class="bg-[#f5f0e8] text-gray-800 antialiased pt-24">



<nav class="fixed top-0 left-0 w-full z-50 px-16 py-3 flex items-center justify-between bg-[#f5f0e8] shadow-md">

    <a href="{{ route('home') }}" class="text-2xl font-black text-[#c17f3a]">
        ISI Burger <span class="text-[#c17f3a]"><i class="fa-solid fa-burger"></i></span>
    </a>

    <div class="flex items-center gap-8">

        <a href="{{ route('home') }}"
           class="text-sm font-semibold {{ request()->routeIs('home') ? 'text-[#c17f3a] border-b-2 border-[#c17f3a]' : 'text-gray-500 hover:text-[#c17f3a]' }}">
            <i class="fa-solid fa-house"></i> Home
        </a>

        <a href="{{ route('menu') }}"
           class="text-sm font-semibold {{ request()->routeIs('menu') ? 'text-[#c17f3a] border-b-2 border-[#c17f3a]' : 'text-gray-500 hover:text-[#c17f3a]' }}">
            <i class="fa-solid fa-utensils"></i> Menu
        </a>

        <a href="{{ route('galerie') }}"
           class="text-sm font-semibold {{ request()->routeIs('galerie') ? 'text-[#c17f3a] border-b-2 border-[#c17f3a]' : 'text-gray-500 hover:text-[#c17f3a]' }}">
            <i class="fa-solid fa-images"></i> Gallery
        </a>

        <a href="{{ route('burgers.index') }}"
           class="text-sm font-semibold {{ request()->routeIs('burgers.index') ? 'text-[#c17f3a] border-b-2 border-[#c17f3a]' : 'text-gray-500 hover:text-[#c17f3a]' }}">
            <i class="fa-solid fa-burger"></i> Burgers
        </a>

        <a href="{{ route('about') }}"
           class="text-sm font-semibold {{ request()->routeIs('about') ? 'text-[#c17f3a] border-b-2 border-[#c17f3a]' : 'text-gray-500 hover:text-[#c17f3a]' }}">
            <i class="fa-solid fa-circle-info"></i> About us
        </a>

        @auth
            <a href="{{ route('client.commandes.index') }}"
               class="text-sm font-semibold {{ request()->routeIs('client.commandes.*') ? 'text-[#c17f3a] border-b-2 border-[#c17f3a]' : 'text-gray-500 hover:text-[#c17f3a]' }}">
    ...
    <div class="flex items-center gap-4">
        ...
        <button onclick="ouvrirPanier()" class="relative text-gray-600 hover:text-[#c17f3a] transition text-xl">
            ...
            <span id="cartCount"
                  class="hidden absolute -top-2 -right-2 bg-[#c17f3a] text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">
                0
            </span>
        </button>
        ...
        @guest
            ...
            <a href="{{ route('register') }}"
               class="bg-[#c17f3a] hover:bg-[#a96d2e] text-white text-sm px-5 py-2 rounded-full transition">
                Inscription
            </a>
        @endguest
...
<main>
    ...
    @if(session('success'))
        <div class="mx-16 mb-4 p-4 bg-green-100 text-green-700 rounded-lg">
            {{ session('success') }}
        </div>
    @endif

    @if(session('error'))
        <div class="mx-16 mb-4 p-4 bg-red-100 text-red-700 rounded-lg">
            {{ session('error') }}
        </div>
    @endif
...
<div class="fixed top-0 right-0 h-full w-80 bg-white z-30 flex flex-col translate-x-full transition-transform duration-300 shadow-2xl"
    ...
    <div class="p-4 border-t">
        <div class="flex justify-between font-bold mb-4">
            <span>Total</span>
            <span id="total" class="text-[#c17f3a] text-xl">FCFA0</span>
        </div>
        ...
        if (panier.length === 0) {
            ...
            for (var i = 0; i < panier.length; i++) {
                ...
                liste.innerHTML +=
                    ...
                    '<button onclick="changerQuantite(' + a.id + ', -1)" class="bg-gray-100 w-7 h-7 rounded">−</button>' +
                    '<span class="w-5 text-center">' + a.quantite + '</span>' +
                    '<button onclick="changerQuantite(' + a.id + ', 1)" class="bg-gray-100 w-7 h-7 rounded">+</button>' +
                    '</div>' +
                    '<button onclick="supprimerArticle(' + a.id + ')" class="text-red-500 ml-2">🗑</button>' +
                    '</div>';
            }
        }
